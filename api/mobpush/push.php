...
class Push
{
    ...
    {
        if (!isset($request['plats']) || !isset($request['target']) || !isset($request['content']) || !isset($request['type']) || !isset($request['appkey'])) {
            throw new Exception('params is not required', 5001);
        }
        /* 必传参数 */
        $requireArr = array('content', 'plats', 'target', 'type', 'appkey');
